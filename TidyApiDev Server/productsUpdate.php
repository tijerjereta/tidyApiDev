<?php 
    ...
    if( $pricingCustomStrn != $pricingCustomMeta ) {
		...
		echo "<br>----<strong>PRICE</strong> UPDATED SUCCESSFULLY";
    ...
    if( $tagName != $recordedTag ) {
		...
		//echo "<br>----<strong>TAG</strong> UPDATED SUCCESSFULLY";
    }
    ...
    if( $name != $product_name ) {
		...
		//echo "<br>----<strong>NAME</strong> UPDATED SUCCESSFULLY";
    }
    // END UPDATE NAME
    //UPDATE MAIN SKU 
    ...
    if( $generalSku != $productSkuRec ) {
		...
		//echo "<br>----<strong>MAIN SKU</strong> UPDATED SUCCESSFULLY";
    }
    //END UPDATE MAIN SKU 
    //UPDATE CATEGORIES
    ...
    if( $categories != $recordedCat ) { 
		...
		//echo "<br>----<strong>CATEGORIES</strong> UPDATED SUCCESSFULLY";
    }
    ...
    if( $imgEntFin != $imgSavedFin) {
        ...
        if( $imgMomento != '' && $imgMomento != ' ' ) { // IF IMAGE FROM TYDY IS NOT EMPTY 
			...
			//echo "<br>----<strong>THUMBNAIL IMAGE</strong> UPDATED SUCCESSFULLY";
    ...
    if( $stockStatus != $regular_stock ) {
		...
		//echo "<br>----<strong>REGULAR STOCK</strong> UPDATED SUCCESSFULLY";
    ...
    if( $desc != $regular_desc ) {
		...
		//echo "<br>----<strong>DESCRIPTION</strong> UPDATED SUCCESSFULLY";	
    }
    ...
    // UPDATE PRODUCTs ATTRIBUTES
        ...
        if( !(empty($product_attributes)) ){
			...
			if( !$result && $countAr1 == $countAr2 ){
                 //echo "<br>LOS A. SON IGUALES LOS ARRAYS<br>";
            }else{
                 //echo "<br>LOS A. NOO SON IGUALES LOS ARRAYS<br>";
                 if( $countAr1 == $countAr2 || $countAr1 > $countAr2 ) {
					 //echo "<br>ENTRA A ESCRIBIR ATRI.<br>";
	   				 echo "<br>PRODUCT: ".$name." UPDATED SUCCESSFULLY".$i;
    ...
    // UPDATE PRODUCTs VARIATIONS
        ...
        if( $product ) {
            if ($product->is_type( 'variable' )) 
            {
        		...
        		//echo "OPTS: <br>";
				//print_r($opts);
				//echo "<br> OPS LEIDAS CANTIDAD: ";
				//print_r(count($opts));
				//echo "<br> VAIRACIONES GUARDADAS CANTIDAD: ";
				//echo "VAIRACIONES DEL GUARDADAS: ";
				//print_r(count($available_variations));
                $writeVariation = 1;
				
				if( count($opts) ==  count($available_variations) || count($opts) >  count($available_variations) ){
					//echo "<br>-*Update Variations only because the ones found are the same amount that the ones that already exist, or bigger.<br>";
					foreach ($available_variations as $variation_values ){
						...
						if( $variation_values['attributes']['attribute_size'] == $opts[$f] ) {
							...
							echo "<br>----VARIATION <strong>SIZE</strong> UPDATED SUCCESSFULLY";
						...
						if( $savedVariationPrc != $producRprice[$f]) {
							...
							echo "<br>----VARIATION <strong>PRICE</strong> UPDATED SUCCESSFULLY";
						}

						if( $skuProduct[$f]!= $savedVariationSku ) {
							update_post_meta( $variation_id, '_sku', $skuProduct[$f] );
							echo "<br>----VARIATION <strong>SKU</strong> UPDATED SUCCESSFULLY";
						}
						if( $savedStock == 1 ) {
							$savedStock = 'instock';
						}else{
							$savedStock = 'outofstock';
						}
						//echo "<br>SAVED STATUS: ".$savedStock."<br>STOCK STATUS".$stockStatusArray[$f]."<br>";
						if( $stockStatusArray[$f] != $savedStock ) {
							update_post_meta( $variation_id, '_stock_status', $stockStatusArray[$f] );
							echo "<br>----VARIATION <strong>STOCK STATUS</strong> UPDATED SUCCESSFULLY";
						}    
